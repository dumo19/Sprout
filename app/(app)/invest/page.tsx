'use client';

import InvestFormCard from '@/app/(app)/invest/components/InvestFormCard';
import InvestmentBreakdownCard from '@/app/(app)/invest/components/InvestmentBreakdownCard';
import InvestmentInfoCard from '@/app/(app)/invest/components/InvestmentInfoCard';
import InvestmentSummaryCard from '@/app/(app)/invest/components/InvestmentSummaryCard';
import { useSession } from '@/context/SessionProvider';
import { Frequency } from '@/types/InvestmentFormProps';
import { formatCurrencyFull } from '@/utils/formatCurrency';
import { useState } from 'react';

export default function InvestPage() {
  const { session, userData } = useSession();
  const [addAmount, setAddAmount] = useState<number>(10);
  const [frequency, setFrequency] = useState<Frequency>('One Time');

  if (!session || !userData) return null;

  return (
    <main className="bg-[#F7F7F2] px-20 py-10 min-h-screen">
      {/* Header */}
      <div className="mb-10 flex flex-row justify-between items-center">
        <div>
          <h1 className="text-4xl">
            Add <span className="text-primary"><i>Money</i>.</span>
          </h1>
          <p>Every dollar you invest today grows for your future.</p>
        </div>
        <div className="flex items-center gap-3 bg-white border-2 border-gray-200 px-3 py-2 rounded-full text-sm">
          <div className="h-2.5 w-2.5 bg-primary rounded-full" />
          <p>Current Balance</p>
          <b>{formatCurrencyFull(userData.portfolio.balance)}</b>
        </div>
      </div>

      {/* Grid */}
      <div className="grid grid-cols-3 gap-8">
        <div className="col-span-2 bg-white border-2 border-gray-200 rounded-2xl">
          <InvestFormCard
            addAmount={addAmount}
            frequency={frequency}
            setAddAmount={setAddAmount}
            setFrequency={setFrequency}
          />
        </div>
        <div className="flex flex-col gap-8">
          <InvestmentSummaryCard addAmount={addAmount} frequency={frequency} />
          <InvestmentBreakdownCard addAmount={addAmount} />
          <InvestmentInfoCard />
        </div>
      </div>
    </main>
  );
}